...
app.controller('homeCtrl', function($location,$window,$ionicSideMenuDelegate,$ionicHistory,$ionicModal,$ionicPlatform,$scope, $state,$stateParams, $ionicPopup, AuthService,$window,$ionicLoading,$q,$ionicSlideBoxDelegate,$cordovaCamera, $cordovaFile, $cordovaFileTransfer,$cordovaDevice, $cordovaActionSheet,$rootScope,$cordovaGeolocation,$timeout,$interval,$ionicScrollDelegate,$translate,$cordovaDevice,$cordovaSocialSharing,$http,$cordovaImagePicker,$anchorScroll) { //,$cordovaOauth
...
.then(function success (newPath) {  
 ...
 //alert(imagePath);
    ...
    // If you are trying to load image from the gallery on Android we need special treatment!
    if ($cordovaDevice.getPlatform() == 'Android' && sourceType === Camera.PictureSourceType.PHOTOLIBRARY) { //alert('file');
      window.FilePath.resolveNativePath(imagePath, function(entry) { //alert(entry);
        window.resolveLocalFileSystemURL(entry, success, fail);
        function fail(e) {
          console.error('Error: ', e);
    ...
    //alert(namePath)
     ...
     var url = $rootScope.serviceurl+"products/product_image_api"
      ...
      $cordovaFileTransfer.upload(url, targetPath, options).then(function(result) {  //alert(JSON.stringify(result))
                console.log(result);
        $scope.ret_img = JSON.parse(result.response);
            $ionicLoading.hide();
        if(id == 1)
    ...
    $scope.not_last = $scope.last_id;
		var friend_id = $scope.friend_id; console.log($scope.last_id);
          ...
          if(result.ACK === 1)  {
            ...
            //var myEl = angular.element( document.querySelector( '#sw_msg.scroll' ) );
     //myEl.append(result.update_message);
                                                                                                        $scope.chat_result.push({status:1,msg:$scope.msg})
	  $scope.msg='';
